...
/* definition of rest mass: Eq. (11) in arXiv:0812.2245v2 M = \int \rho W
 * \sqrt\gamma d^3x this meshes with the EOM for hydro which gives: D_{,t} + (D
 * [\alpha v^i - \beta^i])_{,\i} = 0 (eg. Font lrr-2008-7 Eq. (28) ff) where
 * D=D_{whisky} = \sqrt\gamma*D_{lrr}
 *
 * with this we can easyly get M_{,t} = - \oint_{\partial V} D [\alpha v^i -
 * \beta^i] d\Sigma_i accessing only HydroBase and ADMBase variables this means
 * we need: D = \sqrt\gamma W \rho, \gamma = \sqrt{\det g_{ij}}, W^2 = 1/(1-v^2)
 * this calculation is performed in get_gab_ja_onto_detector below, although I
 * think one should really consider using a generalized get_vars_onto_detector
 * coupled to an expression evaluator later on (roland)
 */
...
/* parse string of variables into array of variable indices (copied from
 * Multipole) */
static void fill_variable(int idx, const char *optstring, void *callback_arg);

/* compute dr/dtheta and dr/dphi */
static int drdth_drdph(int i, int j, int sn, CCTK_REAL dth, CCTK_REAL dph,
                       CCTK_INT verbose, CCTK_INT maxntheta, CCTK_INT maxnphi,
                       CCTK_REAL *sf_radius, CCTK_REAL *ht, CCTK_REAL *hp);

/* call interpolator and interpolate the current density and Lorentz factor onto
 * the detector surface */
static int get_ja_w_eninf_and_extras_onto_detector(
    CCTK_ARGUMENTS, CCTK_INT det, CCTK_INT num_extras,
    CCTK_INT extras_ind[MAX_NUMBER_EXTRAS], CCTK_REAL *jx, CCTK_REAL *jy,
    CCTK_REAL *jz, CCTK_REAL *w, CCTK_REAL *eninf, CCTK_REAL **extras);

/* utility routine to access an element on the interpolated surface */
static int get_j_w_and_eninf_local(int i, int j, int ntheta, CCTK_REAL *j1_det,
                                   CCTK_REAL *j2_det, CCTK_REAL *j3_det,
                                   CCTK_REAL *w_det, CCTK_REAL *eninf_det,
                                   CCTK_REAL jloc[3], CCTK_REAL *wloc,
                                   CCTK_REAL *eninf);
/* utility routine to get value of Cactus parameter
 * surface_projection_<extra_num> */
...
static int Outflow_write_2d_output(CCTK_ARGUMENTS, const char *varname,
                                   CCTK_INT det, const CCTK_REAL *data_det,
                                   const CCTK_REAL *w_det,
                                   const CCTK_REAL *eninf_det,
                                   const CCTK_REAL *surfaceelement_det,
                                   int num_extras, const CCTK_INT *extras_ind,
                                   CCTK_REAL *const extras[]);
...
static int Outflow_write_2d_output(CCTK_ARGUMENTS, const char *varname,
  ...
  if (det >= MAX_NUMBER_DETECTORS) {
    CCTK_VWarn(0, __LINE__, __FILE__, CCTK_THORNSTRING,
               "warn: det=%d, but MAX_NUMBER_DETECTORS=%d, increase", (int)det,
               MAX_NUMBER_DETECTORS);
  }

  // filename
  Util_asprintf(&filename, "%s/outflow_surface_det_%d_%s.asc", out_dir, det,
                varname);
  assert(filename);
  ...
  if (ierr < 0 && ierr != UTIL_ERROR_TABLE_NO_SUCH_KEY) {
    CCTK_VWarn(0, __LINE__, __FILE__, CCTK_THORNSTRING,
               "Internal error: could not check if file '%s' has already been "
               "created: %d",
               filename, ierr);
  }
  ...
  if (!file_created) {
    ...
    fprintf(file,
            "# 1:it 2:t 3:x 4:y 5:z 6:%s 7:w_lorentz 8:eninf 9:surface_element",
            varname);
  ...
  // write data
  len_written = snprintf(format_str_fixed,
                         sizeof(format_str_fixed) / sizeof(format_str_fixed[0]),
                         "%%d\t%%%s\t%%%s\t%%%s\t%%%s\t%%%s\t%%%s\t%%%s\t%%%s",
                         out_format, out_format, out_format, out_format,
                         out_format, out_format, out_format, out_format);
  assert(len_written < sizeof(format_str_fixed) / sizeof(format_str_fixed[0]));
  len_written =
      snprintf(format_str_extras,
               sizeof(format_str_extras) / sizeof(format_str_extras[0]),
               "\t%%%s", out_format);
  assert(len_written <
         sizeof(format_str_extras) / sizeof(format_str_extras[0]));
  ...
  const CCTK_INT sn = sf_IdFromName(surface_index[det], surface_name[det]);
  const CCTK_INT ntheta = sf_ntheta[sn] - 2 * nghoststheta[sn];
  const CCTK_INT imin = nghoststheta[sn],
                 imax = sf_ntheta[sn] - nghoststheta[sn] - 1;
  ...
  for (int i = imin, n = 0; i <= imax;
       i++, n++) { // theta in [0.5 delta_th, pi-0.5 delta_th]
    th = oth + i * dth;
    ct = cos(th);
    st = sin(th);
    ...
    for (int j = jmin, m = 0; j <= jmax; j++, m++) { // phi in [0,2pi-delta_phi]
      ...
      fprintf(file, format_str_fixed, cctk_iteration, cctk_time, det_x, det_y,
              det_z, data_det[ind2d], w_det[ind2d], eninf_det[ind2d],
...
static int Outflow_write_output(CCTK_ARGUMENTS, CCTK_INT det, CCTK_REAL flux,
  ...
  // write data
  sprintf(format_str_real, "%%d\t%%%s\t%%%s\t%%%s\t%%%s", out_format,
...
static CCTK_REAL *get_surface_projection(CCTK_ARGUMENTS, int extra_num) {
  ...
  switch (extra_num) {
  case 0:
    retval = surface_projection_0;
    break;
  case 1:
    retval = surface_projection_1;
    break;
  case 2:
    retval = surface_projection_2;
    break;
  case 3:
    retval = surface_projection_3;
    break;
  case 4:
    retval = surface_projection_4;
    break;
  case 5:
    retval = surface_projection_5;
    break;
  case 6:
    retval = surface_projection_6;
    break;
  case 7:
    retval = surface_projection_7;
    break;
  case 8:
    retval = surface_projection_8;
    break;
  case 9:
    retval = surface_projection_9;
    break;
  case 10:
    retval = surface_projection_10;
    break;
  case 11:
    retval = surface_projection_11;
    break;
  case 12:
    retval = surface_projection_12;
    break;
  case 13:
    retval = surface_projection_13;
    break;
  case 14:
    retval = surface_projection_14;
    break;
  case 15:
    retval = surface_projection_15;
    break;
  case 16:
    retval = surface_projection_16;
    break;
  case 17:
    retval = surface_projection_17;
    break;
  case 18:
    retval = surface_projection_18;
    break;
  case 19:
    retval = surface_projection_19;
    break;
  default:
    CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
...
#undef ALLOCATE_TEMP
  ...
  const void *interp_coords[3] = {(const void *)det_x, (const void *)det_y,
                                  (const void *)det_z};
  ...
  CCTK_STRING input_array_names[NUM_INPUT_ARRAYS] = {
      ...
      "HydroBase::vel[0]", "HydroBase::vel[1]", "HydroBase::vel[2]",
      "HydroBase::rho",

      "ADMBase::betax",    "ADMBase::betay",    "ADMBase::betaz",
      "ADMBase::alp",
  };
  ...
  void *output_arrays[NUM_OUTPUT_ARRAYS + MAX_NUMBER_EXTRAS] = {
      (void *)g11,   (void *)g12,   (void *)g13,   (void *)g22,
      (void *)g23,   (void *)g33,

      (void *)velx,  (void *)vely,  (void *)velz,  (void *)rho0,

      (void *)beta1, (void *)beta2, (void *)beta3, (void *)alpha,
  };
  for (int i = 0; i < num_extras; i++) {
  ...
  if (operator_handle < 0)
    CCTK_VWarn(0, __LINE__, __FILE__, CCTK_THORNSTRING,
               "couldn't find interpolator \"%s\"!", interpolator_name);
  ...
  ierr = CCTK_InterpGridArrays(
      ...
      operator_handle, param_table_handle, coord_system_handle, interp_npoints,
      CCTK_VARIABLE_REAL, interp_coords,
  ...
  for (int i = 0; i < interp_npoints; i++) {
    ...
    detg = 2 * g12[i] * g13[i] * g23[i] +
           g33[i] * (g11[i] * g22[i] - pow2(g12[i])) - g22[i] * pow2(g13[i]) -
           g11[i] * pow2(g23[i]);
    if (detg < 0.) {
      ...
      if (verbose > 1 || (verbose > 0 && last_warned != cctk_iteration)) {
        CCTK_VWarn(
            ...
            "%s: Metric determinant in iteration %6d at %15.6g,%15.6g,%15.6g "
            "is :"
    ...
    if (my_w_lorentz < 1. || v2 > 1) {
      ...
      if (verbose > 1 || (verbose > 0 && last_warned != cctk_iteration)) {
        CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
                   ...
                   "vel = [%15.6g,%15.6g,%15.6g] occured in iteration %d at "
                   "location [%15.6g,%15.6g,%15.6g]",
                   __func__, my_w_lorentz, v2, g11[i], g12[i], g13[i], g22[i],
                   g23[i], g33[i], velx[i], vely[i], velz[i], cctk_iteration,
    ...
    /* - u_t - 1 */
    my_eninf = -my_w_lorentz * (vlowx * beta1[i] + vlowy * beta2[i] +
                                vlowz * beta3[i] - alpha[i]) -
               1.0;
...
static int drdth_drdph(int i, int j, int sn, CCTK_REAL dth, CCTK_REAL dph,
  ...
  ind = (i - 2) + maxntheta * (j + maxnphi * sn); // XXX not sf_ntheta!
  htm2 = sf_radius[ind];
  *ht = (1. / 12. * htm2 - 2. / 3. * htm1 + 2. / 3. * htp1 - 1. / 12. * htp2) /
        dth;
  if (verbose > 5) {
  ...
  ind = i + maxntheta * ((j - 2) + maxnphi * sn); // XXX not sf_ntheta!
  hpm2 = sf_radius[ind];
  *hp = (1. / 12. * hpm2 - 2. / 3. * hpm1 + 2. / 3. * hpp1 - 1. / 12. * hpp2) /
        dph;
  if (verbose > 5) {
...
static CCTK_REAL *j1_det, *j2_det, *j3_det, *w_det, *eninf_det, *fluxdens_det,
    *surfaceelement_det;
static CCTK_INT outflow_get_local_memory(CCTK_INT npoints) {
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
    CCTK_INFO("in allocate_memory");

  if (have_integrand_memory == 0) {
    if (verbose > 0)
      CCTK_INFO("allocating new memory");
    ...
    if (verbose > 1)
      CCTK_INFO("already allocated memory");
    return 2;
  }
...
void outflow(CCTK_ARGUMENTS) {
  ...
  if (!CCTK_Equals(extra_variables, "")) {
    for (int i = 0; i < MAX_NUMBER_EXTRAS;
         i++) /* initialize so that we can count later */
    {
      extras_ind[i] = -1;
    }
    ...
    for (num_extras = 0; num_extras < MAX_NUMBER_EXTRAS;
         num_extras++) /* count valid indices */
    {
  ...
  for (int det = 0; det < num_detectors; det++) {
    ...
    if (sn < 0) {
      CCTK_VWarn(0, __LINE__, __FILE__, CCTK_THORNSTRING,
                 "surface number sn=%d is invalid for detector %d", (int)sn,
                 (int)det);
      continue;
    } else if (sn >= sphericalsurfaces_nsurfaces) {
      CCTK_VWarn(0, __LINE__, __FILE__, CCTK_THORNSTRING,
                 "surface number sn=%d too large, increase "
                 "SphericalSurface::nsurfaces from its current value %d",
                 (int)sn, (int)sphericalsurfaces_nsurfaces);
      continue;
    }
    if (sf_valid[sn] <= 0) {
      CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
                 "didn't find valid detector surface for sn=%d, det=%d",
                 (int)sn, (int)det);
      continue;
    }

    if (nghoststheta[sn] < NGHOSTS ||
        nghostsphi[sn] < NGHOSTS) { // we need at least NGHOSTS ghost zones
      CCTK_VWarn(
          0, __LINE__, __FILE__, CCTK_THORNSTRING,
          "number of ghost zones for spherical surface %d must be at least %d.",
          (int)sn, NGHOSTS);
      continue;
    }
    ...
    const CCTK_INT imin = nghoststheta[sn],
                   imax = sf_ntheta[sn] - nghoststheta[sn] - 1;
    const CCTK_INT jmin = nghostsphi[sn],
                   jmax = sf_nphi[sn] - nghostsphi[sn] - 1;
    ...
    interp_npoints = get_ja_w_eninf_and_extras_onto_detector(
        CCTK_PASS_CTOC, det, num_extras, extras_ind, j1_det, j2_det, j3_det,
        w_det, eninf_det, extras);
    if (interp_npoints < 0) {
      CCTK_WARN(1, "unable to get g_ab, j^a and the extra variables onto the "
                   "detector. not doing anything.");
      continue;
    }
    if (interp_npoints == 0) {
      /* nothing to do (we are not cpu 0) */
      if (verbose > 1) {
        CCTK_VInfo(CCTK_THORNSTRING, "I have nothing to do for detector %d",
                   det);
      }
      continue;
    }
    ...
    CCTK_REAL th, ph;
    CCTK_REAL sum, sum_thresh[MAX_NUMBER_TRESHOLDS], sum_w_lorentz,
        area; // the value of the flux integral
    CCTK_REAL sum_eninf;
         ...
         i++, n++) // theta in [0.5 delta_th, pi-0.5 delta_th]
    {
      ...
      cost = cos(th);
      sint = sin(th);
      // weigths from NR(C++,2) 4.1.14 plus extrapolated 1/2 integral (see Maple
      // worksheet)
      if (i == imin + 0 || i == imax - 0)
        iwtheta = 13.0 / 12.0;
      else if (i == imin + 1 || i == imax - 1)
        iwtheta = 7.0 / 8.0;
      else if (i == imin + 2 || i == imax - 2)
        iwtheta = 25.0 / 24.0;
      else
        iwtheta = 1;
      ...
      for (int j = jmin, m = 0; j <= jmax; j++, m++) // phi in [0,2pi-delta_phi]
      {
        ...
        get_j_w_and_eninf_local(n, m, ntheta, j1_det, j2_det, j3_det, w_det,
                                eninf_det, jloc, &wloc, &eninfloc);

        // the flat space-like 3d unit vectors
        rhat[0] = cosp * sint;
        rhat[1] = sinp * sint;
        rhat[2] = cost;
        thetahat[0] = cosp * cost;
        thetahat[1] = sinp * cost;
        thetahat[2] = -sint;
        phihat[0] = -sinp;
        phihat[1] = cosp;
        phihat[2] = 0;
        ...
        for (int idir = 0; idir < 3; idir++) {
          rdn[idir] = pow2(rp) * sint * rhat[idir] -
                      ht * rp * sint * thetahat[idir] - hp * rp * phihat[idir];
        ...
        fluxdens_det[n + ntheta * m] =
            fluxdens_temp; /* store flux density for output */
    ...
    for (int e = 0; e < num_extras; e++) {
      ...
      for (int i = imin, n = 0; i <= imax;
           i++, n++) // theta in [0.5 delta_th, pi-0.5 delta_th]
      {
        for (int j = jmin, m = 0; j <= jmax;
             j++, m++) // phi in [0,2pi-delta_phi]
        {
          ind = i + maxntheta * (j + maxnphi * det);
          ind2d = n + ntheta * m;
    ...
    ierr = Outflow_write_output(CCTK_PASS_CTOC, det, sum, sum_w_lorentz,
                                sum_eninf, sum_thresh);
    if (ierr < 0) {
      CCTK_WARN(1, "writing of information to files failed");
    }
    if (output_2d_data) {
      ierr = Outflow_write_2d_output(
          CCTK_PASS_CTOC, "fluxdens", det, fluxdens_det, w_det, eninf_det,
          surfaceelement_det, num_extras, extras_ind, extras);
      if (ierr < 0) {
